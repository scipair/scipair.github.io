...
function buildGraph({ authors, collaborators, shared, coauthored }, t) {
  ...
  return { nodes: [...nodes.values()], edges, options };
}

// Re-draws the current layout off-screen in light colours and frames it.
async function exportNetwork(props, positions, { names, fullNames }) {
  const bodyWidth = 1100;
  const bodyHeight = 760;
  // Render at twice the body size in device pixels for a sharp 2x image.
  const ratio = window.devicePixelRatio || 1;
  const { stage, tokens, remove } = lightStage(
    (bodyWidth * 2) / ratio,
    (bodyHeight * 2) / ratio,
  );
  try {
    await fontsReady(tokens);
    const graph = buildGraph(props, tokens);
    ...
    const network = new Network(
      ...
      { ...graph.options, physics: false, interaction: { dragView: false } },
    );
    await new Promise((resolve) => {
      network.once('afterDrawing', resolve);
    ...
    const figure = frame({
      ...
      authors: fullNames,
      legend: [
        { label: `${names[0]}’s coauthors`, color: tokens.a },
        { label: `${names[1]}’s coauthors`, color: tokens.b },
        { label: 'Shared collaborators', color: tokens.both },
      ],
      bodyWidth,
      bodyHeight,
    });
    figure.prims.push({
      type: 'image',
      image: stage.querySelector('canvas'),
      ...figure.body,
      w: bodyWidth,
      h: bodyHeight,
    });
    await savePng(figure, figureName(names, 'collaboration network', 'png'));
    network.destroy();
  } finally {
    remove();
  }
    ...
    <section className="network">
      <header className="network-head">
        ...
        {authors.every(Boolean) && (
          <Download
            label="Download collaboration network"
            onExport={() =>
              exportNetwork(
                { authors, collaborators, shared, coauthored },
                live.current?.getPositions() || {},
                { names, fullNames },
              )
            }
          />
